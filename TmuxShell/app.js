...
wss.on('connection', function connection (ws, req) {
  ...
  if (authError) {
    ...
    // =========================================================================
    // BEGIN MODIFICATION: tmux session routing (NodeGrab + standalone modes)
    //
    // The stock shell app does:
    //   args = dir ? [host, '-t', 'cd ... ; exec $SHELL -l'] : [host];
    //
    // We replace that with two modes selected by query parameters:
    //
    //   ?jobid=XXXXX&ood_session_id=YYYY  (NodeGrab mode)
    //     -> srun into the existing Slurm allocation and attach to the
    //        persistent tmux session that script.sh.erb started in the
    //        batch step cgroup.
    //
    //   ?ood_session_id=YYYY  (standalone mode, no jobid)
    //     -> Start or attach to a tmux session on the login node.
    //        Useful as a drop-in persistent replacement for the stock
    //        shell app.
    //
    //   (no query params at all)
    //     -> Falls through to stock behavior: plain SSH to the host.
    // =========================================================================
    const parsedUrl = url.parse(req.url, true);
    const oodSessionId = parsedUrl.query.ood_session_id || '';
    const jobId = parsedUrl.query.jobid || '';
    const tmuxSessionName = oodSessionId || 'tmux-default';

    if (jobId) {
      // NodeGrab mode: srun into the job allocation, then attach to the
      // persistent tmux session that was started by script.sh.erb in
      // the batch step. The tmux server lives in the job's main cgroup
      // (not an srun step), so it survives when srun connections exit.
      //
      // --overlap allows multiple concurrent srun steps into the same
      // allocation so the user can open several terminals.
      args = [host, '-t', 'bash', '-l', '-c',
        '"srun --jobid=' + jobId + ' --overlap --pty tmux attach-session -t ' + tmuxSessionName + '"'];

      // Mark this WebSocket as Slurm-managed. The ping/pong interval
      // below will skip timeout enforcement for these connections
      // because the session lifetime is controlled by Slurm, not by
      // WebSocket idle timers.
      ws.slurmManaged = true;
    } else if (oodSessionId) {
      // Standalone tmux mode: start or reattach on the login node.
      // -A means "attach if exists, otherwise create new".
      args = [host, '-t', 'tmux', 'new-session', '-A', '-s', tmuxSessionName];
      ws.slurmManaged = false;
    } else {
      // No special params: fall through to stock shell behavior.
...
const interval = setInterval(function ping() {
  wss.clients.forEach(function each(ws) {
    ...
    // =========================================================================
    // BEGIN MODIFICATION: skip timeout enforcement for Slurm-managed sessions.
    // Their lifetime is controlled by the Slurm job, not WebSocket timers.
    // We still do ping/pong keepalive so the connection doesn't go stale.
    // =========================================================================
    if (ws.slurmManaged) {
      if(pingPongEnabled) { ws.isAlive = false; ws.ping(); }
      return;
    }
    // =========================================================================
    // END MODIFICATION
